<?php
	 ...
	 $faq = null;
	 if (!empty($_GET['id'])) {
		 $stmt = $pdo->prepare('SELECT * FROM pm_faqs WHERE id = ?');
	 ...
	 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		 ...
		 if ($question === '') {
			 ...
			 header('Location: pm-faq-edit.php' . ($id ? "?id={$id}" : ''));
			 exit;
		 }

		 if ($id) {
			 $stmt = $pdo->prepare('UPDATE pm_faqs SET question = ?, answer = ? WHERE id = ?');
			 $stmt->execute([$question, $answer, $id]);
			 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'Question updated.'];
		 } else {
			 $max = (int) $pdo->query('SELECT COALESCE(MAX(sort_order), -1) m FROM pm_faqs')->fetch()['m'];
			 $stmt = $pdo->prepare('INSERT INTO pm_faqs (question, answer, sort_order) VALUES (?, ?, ?)');
        ...
        <div class="content-body">
            <div class="container-fluid">
                <div class="page-titles">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="pm-faqs.php">Common Questions</a></li>
                            ...
                            <div class="card-body">
								<form method="post">
									...
									<div class="mb-3">
										...
										<input type="text" name="question" class="form-control" placeholder="Can you manage a rental property for me?" value="<?php echo htmlspecialchars($faq['question']); ?>" required>
									</div>
									<div class="mb-3">
										<label class="form-label">Answer</label>
										<textarea name="answer" class="form-control" rows="4"><?php echo htmlspecialchars($faq['answer']); ?></textarea>
									</div>
									<a href="pm-faqs.php" class="btn btn-outline-secondary">Cancel</a>
