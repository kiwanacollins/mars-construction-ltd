<?php
	 ...
	 $faq = null;
	 if (!empty($_GET['id'])) {
		 $stmt = $pdo->prepare('SELECT * FROM contact_faqs WHERE id = ?');
	 ...
	 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		 ...
		 if ($question === '') {
			 ...
			 header('Location: contact-faq-edit.php' . ($id ? "?id={$id}" : ''));
			 exit;
		 }

		 if ($id) {
			 $stmt = $pdo->prepare('UPDATE contact_faqs SET question = ?, answer = ? WHERE id = ?');
			 $stmt->execute([$question, $answer, $id]);
			 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'Question updated.'];
		 } else {
			 $max = (int) $pdo->query('SELECT COALESCE(MAX(sort_order), -1) m FROM contact_faqs')->fetch()['m'];
			 $stmt = $pdo->prepare('INSERT INTO contact_faqs (question, answer, sort_order) VALUES (?, ?, ?)');
        ...
        <div class="content-body">
            <div class="container-fluid">
                <div class="page-titles">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="contact-faqs.php">Common Questions</a></li>
                            ...
                            <div class="card-body">
								<form method="post">
									...
									<div class="mb-3">
										...
										<input type="text" name="question" class="form-control" placeholder="What factors should I consider when buying a home?" value="<?php echo htmlspecialchars($faq['question']); ?>" required>
									</div>
									<div class="mb-3">
										<label class="form-label">Answer</label>
										<textarea name="answer" class="form-control" rows="4"><?php echo htmlspecialchars($faq['answer']); ?></textarea>
									</div>
									<a href="contact-faqs.php" class="btn btn-outline-secondary">Cancel</a>
