<?php
	 ...
	 $stat = null;
	 if (!empty($_GET['id'])) {
		 $stmt = $pdo->prepare('SELECT * FROM pm_stats WHERE id = ?');
	 ...
	 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		 ...
		 if ($label === '') {
			 ...
			 header('Location: pm-stat-edit.php' . ($id ? "?id={$id}" : ''));
			 exit;
		 }

		 if ($id) {
			 $stmt = $pdo->prepare('UPDATE pm_stats SET value = ?, suffix = ?, label = ? WHERE id = ?');
			 $stmt->execute([$value, $suffix, $label, $id]);
			 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'Stat updated.'];
		 } else {
			 $max = (int) $pdo->query('SELECT COALESCE(MAX(sort_order), -1) m FROM pm_stats')->fetch()['m'];
			 $stmt = $pdo->prepare('INSERT INTO pm_stats (value, suffix, label, sort_order) VALUES (?, ?, ?, ?)');
        ...
        <div class="content-body">
            <div class="container-fluid">
                <div class="page-titles">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="pm-stats.php">Why Owners Choose Us</a></li>
                            ...
                            <div class="card-body">
								<form method="post">
									...
									<div class="mb-3">
										...
										<input type="text" name="label" class="form-control" placeholder="Response Rate" value="<?php echo htmlspecialchars($stat['label']); ?>" required>
									</div>
									<a href="pm-stats.php" class="btn btn-outline-secondary">Cancel</a>
