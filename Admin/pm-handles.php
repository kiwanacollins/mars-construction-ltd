<?php
	 ...
	 if (!empty($_GET['delete'])) {
		 $pdo->prepare('DELETE FROM pm_handles WHERE id = ?')->execute([(int) $_GET['delete']]);
		 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'Item deleted.'];
		 header('Location: pm-handles.php');
		 exit;
	 }

	 $handles = $pdo->query('SELECT * FROM pm_handles ORDER BY sort_order, id')->fetchAll();

	 function pm_handle_image_src($path) {
		 if (empty($path)) {
			 return null;
		 }
        ...
        <div class="content-body">
            <div class="container-fluid">
				...
				<div class="clean-list-header">
					...
					<a href="pm-handle-edit.php" class="clean-list-btn-add"><i class="fa fa-plus"></i>Add Item</a>
				</div>
					...
					<?php else: ?>
						<div class="table-responsive">
							<table class="clean-list-table">
								...
								<tbody>
									<?php foreach ($handles as $h): $img = pm_handle_image_src($h['image']); ?>
										<tr>
											<td>
												<?php if ($img): ?>
											...
											<td><a href="pm-handle-edit.php?id=<?php echo $h['id']; ?>" class="clean-list-title"><?php echo htmlspecialchars($h['title']); ?></a></td>
											<td>
												<div class="clean-list-actions">
													<a href="pm-handle-edit.php?id=<?php echo $h['id']; ?>" class="edit-btn" title="Edit"><i class="fa fa-pen"></i></a>
													<a href="pm-handles.php?delete=<?php echo $h['id']; ?>" class="delete-btn" title="Delete" onclick="return confirm('Delete this item?');"><i class="fa fa-trash"></i></a>
												</div>
											</td>
										</tr>
									<?php endforeach; ?>
