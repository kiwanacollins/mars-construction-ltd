<?php
	 ...
	 if (!empty($_GET['delete'])) {
		 $pdo->prepare('DELETE FROM construction_stats WHERE id = ?')->execute([(int) $_GET['delete']]);
		 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'Stat deleted.'];
		 header('Location: construction-stats.php');
		 exit;
	 }

	 $stats = $pdo->query('SELECT * FROM construction_stats ORDER BY sort_order, id')->fetchAll();
            ...
            <div class="container-fluid">
                <div class="page-titles">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active"><a href="javascript:void(0)">Project Track Record</a></li>
					</ol>
                </div>
				...
				<div class="clean-list-header">
					...
					<a href="construction-stat-edit.php" class="clean-list-btn-add"><i class="fa fa-plus"></i>Add Stat</a>
				</div>
								...
								<tbody>
									<?php foreach ($stats as $s): ?>
										<tr>
											...
											<td><a href="construction-stat-edit.php?id=<?php echo $s['id']; ?>" class="clean-list-title"><?php echo htmlspecialchars($s['label']); ?></a></td>
											<td>
												<div class="clean-list-actions">
													<a href="construction-stat-edit.php?id=<?php echo $s['id']; ?>" class="edit-btn" title="Edit"><i class="fa fa-pen"></i></a>
													<a href="construction-stats.php?delete=<?php echo $s['id']; ?>" class="delete-btn" title="Delete" onclick="return confirm('Delete this stat?');"><i class="fa fa-trash"></i></a>
												</div>
											</td>
										</tr>
									<?php endforeach; ?>
