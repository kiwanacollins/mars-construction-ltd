<?php
	 ...
	 if (!empty($_GET['delete'])) {
		 $pdo->prepare('DELETE FROM construction_faqs WHERE id = ?')->execute([(int) $_GET['delete']]);
		 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'FAQ deleted.'];
		 header('Location: construction-faqs.php');
		 exit;
	 }

	 $faqs = $pdo->query('SELECT * FROM construction_faqs ORDER BY sort_order, id')->fetchAll();
        ...
        <div class="content-body">
            <div class="container-fluid">
				...
				<div class="clean-list-header">
					...
					<a href="construction-faq-edit.php" class="clean-list-btn-add"><i class="fa fa-plus"></i>Add Question</a>
				</div>
							...
							<table class="clean-list-table">
								...
								<tbody>
									<?php foreach ($faqs as $f): ?>
										<tr>
											<td><a href="construction-faq-edit.php?id=<?php echo $f['id']; ?>" class="clean-list-title"><?php echo htmlspecialchars($f['question']); ?></a></td>
											<td><?php echo htmlspecialchars(mb_strimwidth($f['answer'] ?? '', 0, 60, '...')); ?></td>
											<td>
												<div class="clean-list-actions">
													<a href="construction-faq-edit.php?id=<?php echo $f['id']; ?>" class="edit-btn" title="Edit"><i class="fa fa-pen"></i></a>
													<a href="construction-faqs.php?delete=<?php echo $f['id']; ?>" class="delete-btn" title="Delete" onclick="return confirm('Delete this question?');"><i class="fa fa-trash"></i></a>
												</div>
											</td>
										</tr>
									<?php endforeach; ?>
