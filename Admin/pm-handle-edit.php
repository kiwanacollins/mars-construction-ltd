<?php
	 ...
	 $handle = null;
	 if (!empty($_GET['id'])) {
		 $stmt = $pdo->prepare('SELECT * FROM pm_handles WHERE id = ?');
	 ...
	 if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		 ...
		 if ($title === '') {
			 ...
			 header('Location: pm-handle-edit.php' . ($id ? "?id={$id}" : ''));
			 exit;
		 }
		 ...
		 if ($id) {
			 $stmt = $pdo->prepare('UPDATE pm_handles SET title = ?, image = ? WHERE id = ?');
			 $stmt->execute([$title, $image, $id]);
			 $_SESSION['form_status_message'] = ['type' => 'success', 'text' => 'Item updated.'];
		 } else {
			 $max = (int) $pdo->query('SELECT COALESCE(MAX(sort_order), -1) m FROM pm_handles')->fetch()['m'];
			 $stmt = $pdo->prepare('INSERT INTO pm_handles (title, image, sort_order) VALUES (?, ?, ?)');
        ...
        <div class="content-body">
            <div class="container-fluid">
                <div class="page-titles">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="pm-handles.php">What We Handle</a></li>
                            ...
                            <div class="card-body">
								<form method="post" enctype="multipart/form-data">
									...
									<div class="mb-3">
										...
										<input type="text" name="title" class="form-control" placeholder="Routine maintenance and seasonal upkeep" value="<?php echo htmlspecialchars($handle['title']); ?>" required>
									</div>
									<div class="mb-3">
									...
									<a href="pm-handles.php" class="btn btn-outline-secondary">Cancel</a>
